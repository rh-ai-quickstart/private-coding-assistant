...
When guardrails is on, OpenCode chat still uses `maas-default-gateway`. HTTPRoute sends `/v1/chat/completions` to `guardrails-proxy` after API-key auth. A prompt with a gitleaks-shaped secret is blocked; a clean prompt still completes.

## Sub-features

- `gr-url` workspace `OPENAI_BASE_URL` contains `maas-default-gateway` (not `guardrails-proxy`).
...
- DevSpace with `guardrails.enabled` on ai-serving (chart default). OpenCode uses doctor `IDE_BASE_URL` (`maas-default-gateway`, class from the live Gateway).
...
- In `MODE=ai`, Service `guardrails-proxy` must exist. In `MODE=devspace`, OpenCode secret-block is enough (e2e `require_opencode_guardrails_url` checks `maas-default-gateway`).
- OpenCode secret `opencode-web-password` present. Command starts a Stopped workspace.

- **URL.** `devspace-status` (or doctor) shows `IDE_BASE_URL` with `maas-default-gateway` and `IDE_VIA=gateway`. `API_KEY_PRESENT=true`.
