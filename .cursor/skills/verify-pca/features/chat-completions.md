...
- `chat-gateway` completes a short prompt through MaaS/RHCL with a per-DevSpace API key and `X-PCA-*` headers.
...
- Log in as `dev-userN`, open Dev Spaces, start `code-workspace-1`, chat in OpenCode. Workspace `OPENAI_BASE_URL` is doctor `IDE_BASE_URL` (MaaS Gateway ClusterIP or public hostname — class comes from the live Gateway, often `openshift-default` on existing OpenShift, not always `data-science-gateway-class`).
...
- Break-glass: point the IDE at doctor `LLMD_GATEWAY_V1` when `aiGateway.escapeHatchToLlmd=true`.
...
- Default `chat` in `MODE=ai` hits MaaS. Use `VIA=opencode` when proving the workspace Web API. Use `VIA=llmd` only for the escape hatch.
- Gateway chat without `DEV_USER`/`DEV_NAMESPACE` fails looking up secret `pca-maas-apikey`. That is a missing precondition, not a product outage.
- If `AI_GATEWAY_PRESENT=false`, report `chat-gateway` unreachable. Do not count `VIA=llmd` or OpenCode as MaaS proof.
...
- OpenCode has one `OPENAI_BASE_URL` (`/v1`). Continue tab autocomplete uses `/local/v1` (skips guardrails and Semantic Router).
