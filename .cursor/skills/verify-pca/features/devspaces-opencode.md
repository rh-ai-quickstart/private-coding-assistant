...
- `opencode-e2e` (optional deep) runs the calculator agent path.
...
- **Deep agent (optional).** `make e2e DEV_USER=<same user>` — OpenCode must already be deployed; missing OpenCode is a failure, not a skip. Use when the change is in the OpenCode image or workspace wiring.
- **Proof.** `dw-exists` + `opencode-route` are the default bar for "workspace is deployed". Chat from IDE is `opencode-chat`, not `devspace-status` alone. If you skip live Web or calculator e2e, say so.
